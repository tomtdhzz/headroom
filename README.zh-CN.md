...
**切换时看花费与适配。** TUI 策略组面板里每个节点右侧显示**价格**(输出价,单位 **$/百万 token**;绿=便宜/灰=中/紫=贵),以及能力标签 `👁 看图`(能读图)、`🧠 推理`;选中节点时底部给出完整报价(输入/输出,含单位)、上下文大小,以及**适用场景**(`适合:看图/截图、推理/规划、长文/大仓库、日常/轻量`——由能力字段推导);再下面一行是**任务推荐**——`💡 省 / 看图 / 推理 / 长文` 各推一个最合适的模型。按 `t` 可按价格排序,额度告警时一眼挑到更省的替代模型。**监控主视图**每行也会显示钉到该 provider 的当前模型(`▸ 在用 default:opus-4-8`)。
